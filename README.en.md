...
- **Model tools**: 14 tools — search (with citations), list/create/delete bases, add/list/delete documents, import URL, refresh URL, stats, get document, deep-read a document (paged text slices or regex grep), reindex one document or a whole base. Every read/write tool and proactive retrieval share the strict enabled-base boundary; empty/stale selections cannot widen to all bases, permanent deletion requires host approval, and long reads report their continuation offset.
...
dsh plugin --profile <name> add ./dsh-knowledge-0.3.6.tgz
```
