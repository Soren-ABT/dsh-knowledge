...
export const zh: Record<KnowledgeKey, string> = {
  ...
  localWorkerIdleTimeoutMs: '本地模型 worker 空闲超时（毫秒，0 = 常驻）',
  localWorkerIdleTimeoutMsHint: '本地嵌入模型在空闲该时长后被释放以释放内存，下次请求重新拉起；重新拉起在 Linux 上可能触发 onnxruntime 绑定重载失败（Module did not self-register）。0 = 常驻（模型常热、无重载风险，仅占内存）；调大值可减少重拉次数',
...
export const en: Record<KnowledgeKey, string> = {
  ...
  localWorkerIdleTimeoutMs: 'Local-model worker idle timeout (ms, 0 = keep alive)',
  localWorkerIdleTimeoutMsHint: 'The local embedding worker is released after this much idle time to free memory; the next request respawns it, and a respawn can fail on Linux with "Module did not self-register" (onnxruntime binding reload). 0 = keep alive (model stays hot, no respawn risk, costs resident memory); a larger value reduces respawns',
